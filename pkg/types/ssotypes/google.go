...
func (g *GoogleOAuthProvider) createIdentity(ctx context.Context, token *oauth2.Token) (identity *SSOIdentity, err error) {
	...
	// For generic OIDC providers (like Keycloak), there might not be a 'hd' claim.
	// We fall back to verifying the email domain suffix.
	if g.HostedDomain != "" && claims.HostedDomain != g.HostedDomain {
		if !strings.HasSuffix(claims.Email, "@"+g.HostedDomain) {
			return identity, fmt.Errorf("oidc: unexpected hd claim %v or email domain mismatch for %v", claims.HostedDomain, claims.Email)
		}
	}
